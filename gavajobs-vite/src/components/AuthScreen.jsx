...
export default function AuthScreen({ onAuth }) {
  ...
  const inputFocus = { borderColor:C.red }
        ...
        <div style={{ width:"100%", maxWidth:420 }}>
          ...
          <div style={{ textAlign:"center", marginBottom:32 }}>
            ...
            <div style={{ fontSize:14, color:C.text2, lineHeight:1.5 }}>
              Kenya's government job matching portal
            </div>
          </div>
              ...
              {["login","register"].map(m => (
                <button key={m} onClick={() => { setMode(m); setError("") }}
                  style={{ flex:1, padding:"10px 0", fontSize:14, fontWeight:600, fontFamily:"inherit", border:"none", borderRadius:8, cursor:"pointer",
                    ...
                    boxShadow:mode===m?"0 1px 4px rgba(0,0,0,0.08)":"none", transition:"all 0.2s" }}>
            ...
            <form onSubmit={handleSubmit}>
              ...
              {mode === "register" && (
                <div style={{ marginBottom:14 }}>
                  ...
                  <input value={name} onChange={e => setName(e.target.value)} placeholder="John Kamau"
                    style={inputStyle} onFocus={e => Object.assign(e.target.style, inputFocus)} onBlur={e => e.target.style.borderColor=C.border} />
                </div>
              )}
              <div style={{ marginBottom:14 }}>
              ...
              <button type="submit" disabled={loading}
                style={{ width:"100%", padding:"13px 0", fontSize:15, fontWeight:700, fontFamily:"inherit", border:"none", borderRadius:10, cursor:loading?"wait":"pointer",
                  background:loading?C.text3:C.red, color:C.white, transition:"background 0.2s", marginTop:4 }}>
